import 'package:flutter/material.dart';

class AppColor {
  static const Color lightGrey = Color.fromRGBO(126, 124, 125, 1);
  static const Color black = Color.fromRGBO(0, 0, 0, 1);
  static const Color black36 = Color.fromRGBO(0, 0, 0, 0.36);
  static const Color black50 = Color.fromRGBO(0, 0, 0, 0.5);
  static const Color white = Color.fromRGBO(255, 255, 255, 1);
  static const Color backgroundIndicator40 = Color.fromRGBO(253, 253, 253, 0.4);
  static const Color transparent = Color.fromRGBO(0, 0, 0, 0);
  static const Color blueFirst = Color.fromRGBO(1, 124, 250, 1);
  static const Color blueSecond = Color.fromRGBO(1, 126, 255, 1);
  static const Color blueSecond85 = Color.fromRGBO(1, 126, 255, 0.85);
  static const Color lightBlue = Color.fromRGBO(147, 200, 255, 1);
  static const Color skyBlue = Color.fromRGBO(105, 178, 255, 1);
  static const Color textGrey = Color.fromRGBO(112, 112, 112, 1);
  static const Color textGrey50 = Color.fromRGBO(112, 112, 112, 0.5);
  static const Color textGrey30 = Color.fromRGBO(112, 112, 112, 0.3);
  static const Color errorMessage = Color.fromRGBO(217, 54, 54, 1);
  static const Color error = Color.fromRGBO(178, 27, 27, 1);
  static const Color blackBlue = Color.fromRGBO(1, 37, 63, 1);
  static const Color backgroundGrey = Color.fromRGBO(228, 228, 228, 1);
  static const Color backgroundMoreGrey = Color.fromRGBO(193, 193, 193, 1);
  static const Color backgroundLittleGrey = Color.fromRGBO(206, 205, 205, 1);
  static const Color dimBlack = Color.fromRGBO(35, 31, 32, 1);

  static const Color titleNotification = Color.fromRGBO(209, 225, 255, 1);

}